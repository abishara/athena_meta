...
def trim_reads(
  ...
  def trim3p(seq): return seq[:-500]
  def trim5p(seq): return seq[500:]

  with open(outfa_path, 'w') as fout:
    for e in util.fa_iter(infa_path):
      seq = e.seq
      header_aux = ''
      if e.qname not in qname_3p_overlaps:
        seq = trim3p(seq)
        header_aux += ' trim3p'
      if e.qname not in qname_5p_overlaps:
        seq = trim5p(seq)
        header_aux += ' trim5p'
...
def get_overlaps(canu_path):
  readnames_path = os.path.join(
    canu_path,
    'unitigging/canu.gkpStore/readNames.txt',
  ) 
  edges_path = os.path.join(
    canu_path,
    'unitigging/4-unitigger/canu.best.edges',
  )
  rid_map = {}
